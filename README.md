...
https://RohKJ.github.io/dog_random_ear/dog_ear_roulette.html?sample=front
...
실행 파일은 로컬 서버를 열어 앱을 실행합니다. 샘플 사진과 자동 맞춤 기능을 안정적으로 쓰려면 HTML 파일을 직접 여는 것보다 이 방식이 좋습니다.
...
4. COCO-SSD가 놓치는 사진은 눈/코처럼 어두운 얼굴 포인트를 추가로 분석합니다.
5. 귀 또는 모자 장식이 릴스처럼 위에서 아래로 내려옵니다.
6. 사용자가 원하는 순간 멈추면 해당 프레임 그대로 장식을 고정합니다.
7. 사용자가 최종 이미지를 PNG로 저장합니다.
...
멈춘 순간의 프레임을 그대로 유지해 릴스 영상처럼 자연스럽게 고정되는 느낌을 강화합니다.
